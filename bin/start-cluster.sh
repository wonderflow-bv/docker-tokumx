...
SHARD00_ID=$(sudo docker run -dt $IMAGE mongod --replSet rs0 --shardsvr --bind_ip 0.0.0.0 --port 10000)
...
SHARD01_ID=$(sudo docker run -dt $IMAGE mongod --replSet rs0 --shardsvr --bind_ip 0.0.0.0 --port 10001)
...
SHARD02_ID=$(sudo docker run -dt $IMAGE mongod --replSet rs0 --shardsvr --bind_ip 0.0.0.0 --port 10002)
...
CONFIG0_ID=$(sudo docker run -d $IMAGE mongod --configsvr  --dbpath /data/ --bind_ip 0.0.0.0 --port 10000)
...
MONGOS0_ID=$(sudo docker run -p 9999:9999 -d $IMAGE mongos --configdb ${CONFIG0_IP}:10000 --bind_ip 0.0.0.0 --port 9999)
